...
function createTodoElement(todo) {
    ...
    listItem.className = `todo-item priority-${todo.priority}`;
    if (todo.completed) {
    ...
    priorityBadge.innerText = getPriorityLabel(todo.priority);
    ...
    deleteBtn.value = "Entfernen";
    deleteBtn.type = "button";
    deleteBtn.addEventListener('click', _ => onDeleteButtonClicked(todo));

    listItem.append(cbDone, priorityBadge, todoText, deleteBtn);
    return listItem;
}

function getPriorityLabel(priority) {
    switch (priority) {
        case 1: return "DRINGEND";
        case 2: return "HOCH";
...
function render() {
    ...
    // Sort logic: 1 is highest, 5 is lowest
...
function onAddButtonClicked() {
    ...
    if (text !== "") {
        const priority = parseInt(newTodoPriority.value);
        const ti = new TodoItem(text, false, priority);
        addTodoItem(ti);
        newTodoText.value = "";
        render();
    }
